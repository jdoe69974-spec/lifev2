...
export default function Index() {
  ...
  // NEW: State to track our safety cooldown
  const [isCoolingDown, setIsCoolingDown] = useState(false);

  const recognitionRef = useRef<any>(null);

  useEffect(() => {
    // Only reset status to default if we aren't actively cooling down
    if (!isCoolingDown) {
  ...
  const handleProcess = useCallback(async (text: string) => {
    ...
    try {
      ...
      // --- THE 5-SECOND SAFETY COOLDOWN ---
      setIsCoolingDown(true);
      setStatus(lang === 'es' ? 'Enfriamiento de seguridad (5s)...' : 'Safety Cooldown (Wait 5s)...');
      
      setTimeout(() => {
        setIsCoolingDown(false);
        // The useEffect will automatically catch this state change and reset the default text!
      }, 5000);
    }
  }, [lang, county, detailLevel, getFullContext, isProcessing, isCoolingDown]);

  const toggleRecording = useCallback(() => {
    // NEW: Block all recording attempts if we are currently cooling down or processing
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-4">
                ...
                <div className="flex flex-col justify-end">
                  ...
                  <select
                    ...
                    // Disable the dropdown while processing or cooling down to prevent mid-flight changes
            ...
            <AIGuidance
              ...
              isPlaying={isPlaying}
              canPlay={!!recommendation && !isCoolingDown} // Don't let them replay audio during cooldown
              onPlay={handlePlay}
            />
          </div>
